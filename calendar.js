...
function getCalendarDays(month, year) {
    ...
    const lastDayOfWeekMonth = lastDayOfMonth.getDay();
    const endShift = lastDayOfWeekMonth === 0 ? 0 : 7 - lastDayOfWeekMonth;

    for (
        let currentDateNumber = 1 - startShift;
        currentDateNumber <= lastDayOfMonthNumber + endShift; 
        currentDateNumber++ ) {
        days.push(new Date(year, month, currentDateNumber));
    }
...
function createDayEl(calendarDay, notInCurrentMonth, current, display) {
    ...
    linkEl.href = `?day=${data2String(calendarDay)}`;
...
function renderTitle(calendarMonth, calendarYear) {
    calendarDayTitle.innerText = `${MONTHS[calendarMonth]} ${calendarYear}`;
}
